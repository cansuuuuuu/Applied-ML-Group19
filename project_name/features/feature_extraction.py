...
class FeatureExtraction:
    ...
    def split_channels(self, image: np.ndarray) \
    ...
    def channel_std_deviation(self, image: np.ndarray) -> float:
        """
        Returns the the standard deviation of the pixel valeus
        for a given channel
        Args:
            image (mv): vector of pixel values for a color channel
        Return:
    ...
    def run(self, image) -> Dict[str, float]:
        """
        Returns the extracted features for the input file
        in the form of a Dictionary
        Args:
            file_path (str) : path the the file to be analyzed
        Return:
            feats (Dict[str, float]) : dict of the features
        ...
        glcm = feat.graycomatrix(b_channel,
                                 distances=[1],

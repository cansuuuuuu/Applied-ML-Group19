...
from project_name.data.loading import load_dataset
...
class SVMModel:
    def __init__(self, model_path="project_name/models/svm_model.pkl", vectorizer_path="project_name/models/vectorizer.pkl"):
        self.model_path = model_path
        self.vectorizer_path = vectorizer_path
        self.model = None
        self.vectorizer = None

    def flatten_pixels(self, X_train, X_test):
        """
        ...
        X_train_flat = to_gray(X_train).reshape(
            len(X_train), -1).astype(np.float32) / 255.0
        X_test_flat = to_gray(X_test).reshape(
            len(X_test), -1).astype(np.float32) / 255.0

        return X_train_flat, X_test_flat

    def hoggify(self, imgs):
        """
    ...
    def build_pipeline(self) -> GridSearchCV:
        """
        ...
        parameter_grid = {
            'pca__n_components': [0.8, 0.85, 0.9, 0.95, 0.99, None],
            'svm__C': [0.1, 1, 10, 100],
    ...
    def fit(self, X_train, y_train):
        """
        Fits the SVM model using GridSearch.
        :param X_train: the training set.
        :param y_train: the test labels
        """
        grid = self.build_pipeline()
    ...
    def show_hog_example(self, image_path):
        """
...
def run_pipeline():
    ...
    # Save vectorizer names as in the original code
    vectorizer_names = vectorizer.get_feature_names_out()
    joblib.dump(vectorizer_names, svm_model.vectorizer_path)

    svm_model.evaluate(X_test_feats, y_test, classes)
    # svm_model.show_hog_example(TRAIN_DIR/"1_cumulus/1_cumulus_000009.jpg")
